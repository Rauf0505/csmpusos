...
export default function DeptReports() {
  return <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="p-6 md:p-8 max-w-4xl">
    <h1 className="font-heading text-[22px] font-semibold text-foreground mb-1">Reports</h1>
    <p className="text-sm text-muted-foreground mb-6">Department analytics and reports</p>
    <div className="bg-white border border-border/80 rounded-lg p-12 text-center">
      ...
      <div className="flex items-center justify-center gap-2 mt-5 text-[11px] text-muted-foreground">
        <Clock size={12} /><span>Generate reports on student performance, faculty, and courses</span>
      </div>
    </div>
  </motion.div>;
}
